...
import 'package:ahwa_manager/services/report_manager.dart';
  ...
  final orderManager = OrderManager(orderRepo);

  // Add orders
  orderManager.addOrder('Mohamed', const Shai(), 'Extra mint, ya rais');
  orderManager.addOrder('eslam', const TurkishCoffee(), 'No sugar');
  orderManager.addOrder('Youssef', const HibiscusTea(), 'Cold');

  // Complete some orders
  var pending = orderManager.getPendingOrders();
  orderManager.completeOrder(pending.first);

  // Reports
  final report = ReportGenerator(orderManager.allOrders);

  print('--- Pending Orders ---');
  for (var o in orderManager.getPendingOrders()) {
    print('${o.customerName} ordered ${o.drink.name} (${o.instructions})');
  }
